...
public interface MessageRepository extends JpaRepository<Message, Long> {
     ...
     */
    @Query(value = "SELECT * FROM (SELECT * FROM Messages m WHERE m.sender_id=:user0 AND m.receiver_id=:user1 " +
            "UNION SELECT * FROM Messages m WHERE m.sender_id=:user1 AND m.receiver_id=:user0) ORDER BY timestamp ASC",
            nativeQuery=true)
    List<Message> findMessagesBetweenUsers(@Param("user0") Long user0Id, @Param("user1") Long user1Id);

    @Transactional
    @Modifying
    @Query(value = "UPDATE Message m set m.isRead=true WHERE m.receiver.id=:userId AND m.sender.id=:buddyUserId " +
            "AND m.isRead=false")
    void updateUnreadMessagesForAUser(@Param("userId") Long userId, @Param("buddyUserId") Long buddyUserId);

    @Transactional
    @Modifying
    @Query(value = "INSERT INTO Messages (sender_id, receiver_id, timestamp, content) " +
            "VALUES (:senderId, :receiverId, CURRENT_TIMESTAMP(), :message)", nativeQuery=true)
    void createMessage(@Param("senderId") Long senderId,
     ...
     */
    @Query(value = "SELECT * FROM (SELECT * FROM Messages m WHERE m.sender_id=:user0 AND m.receiver_id=:user1 " +
            "UNION SELECT * FROM Messages m WHERE m.sender_id=:user1 AND m.receiver_id=:user0) " +
